...
function test_complex_security_group() {
  ...
  echo "##### scenario: 3. ins-A:Hostにsg-Aを割り当てる。"
  security_group_id=$sg_A
  run_cmd network_vif add_security_group $vif_a
  echo press ctrl-D to continue tests;cat

  echo "##### scenario: 4. ins-B:Hostにsg-Aを割り当てる。"
  security_group_id=$sg_A
  run_cmd network_vif add_security_group $vif_b
  echo press ctrl-D to continue tests;cat

  echo "##### scenario: 5. ins-Bを破棄する。"
  ssh_key_pair_uuid="$(cached_instance_param ${instance_b}   | egrep ' ssh-' | awk '{print $2}')"
  security_group_uuid="$(cached_instance_param ${instance_b} | egrep ' sg-'  | awk '{print $2}')"
  destroy_instance
  echo press ctrl-D to continue tests;cat

  echo "##### scenario: 6. ins-B(同一IP)で起動する。"
  host_group=gsvb create_instance
  instance_b=$instance_uuid
  echo ${instance_b} >> ${instance_uuids_path}
  echo "$(cached_instance_param ${instance_b})"
  local instance_ipaddr="$(cached_instance_param ${instance_b} | hash_value address)"
  wait_for_network_to_be_ready ${instance_ipaddr}
  wait_for_sshd_to_be_ready    ${instance_ipaddr}
  echo ====================
  echo i-B
